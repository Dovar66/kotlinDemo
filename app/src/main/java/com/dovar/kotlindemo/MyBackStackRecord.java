...
public class MyBackStackRecord extends MyFragmentTransaction implements
        MyFragmentManagerImp.BackStackEntry, MyFragmentManagerImp.OpGenerator {


    static final String TAG = MyFragmentManagerImp.TAG;
    static final boolean SUPPORTS_TRANSITIONS = Build.VERSION.SDK_INT >= 21;

    final MyFragmentManagerImp mManager;
    ...
    public MyBackStackRecord(MyFragmentManagerImp manager) {
        mManager = manager;
    }
    ...
    @Override
    public MyFragmentTransaction add(MyFragment fragment, String tag) {
        doAddOp(0, fragment, tag, OP_ADD);
        return this;
    }

    @Override
    public MyFragmentTransaction add(int containerViewId, MyFragment fragment) {
        doAddOp(containerViewId, fragment, null, OP_ADD);
        return this;
    }

    @Override
    public MyFragmentTransaction add(int containerViewId, MyFragment fragment, String tag) {
    ...
    @Override
    public MyFragmentTransaction replace(int containerViewId, MyFragment fragment) {
        return replace(containerViewId, fragment, null);
    }

    @Override
    public MyFragmentTransaction replace(int containerViewId, MyFragment fragment, String tag) {
        if (containerViewId == 0) {
    ...
    @Override
    public MyFragmentTransaction remove(MyFragment fragment) {
        Op op = new Op();
        op.cmd = OP_REMOVE;
        op.fragment = fragment;
        addOp(op);
    ...
    @Override
    public MyFragmentTransaction hide(MyFragment fragment) {
        Op op = new Op();
        op.cmd = OP_HIDE;
        op.fragment = fragment;
        addOp(op);
    ...
    @Override
    public MyFragmentTransaction show(MyFragment fragment) {
        Op op = new Op();
        op.cmd = OP_SHOW;
        op.fragment = fragment;
        addOp(op);
    ...
    @Override
    public MyFragmentTransaction detach(MyFragment fragment) {
        Op op = new Op();
        op.cmd = OP_DETACH;
        op.fragment = fragment;
        addOp(op);
    ...
    @Override
    public MyFragmentTransaction attach(MyFragment fragment) {
        Op op = new Op();
        op.cmd = OP_ATTACH;
        op.fragment = fragment;
        addOp(op);
    ...
    @Override
    public MyFragmentTransaction setCustomAnimations(int enter, int exit) {
    ...
    @Override
    public MyFragmentTransaction setTransition(int transition) {
        mTransition = transition;
        return this;
    }

    @Override
    public MyFragmentTransaction addSharedElement(View sharedElement, String name) {
    ...
    @Override
    public MyFragmentTransaction setTransitionStyle(int styleRes) {
        mTransitionStyle = styleRes;
        return this;
    }

    @Override
    public MyFragmentTransaction addToBackStack(String name) {
    ...
    @Override
    public MyFragmentTransaction disallowAddToBackStack() {
        if (mAddToBackStack) {
            throw new IllegalStateException(
    ...
    @Override
    public MyFragmentTransaction setBreadCrumbTitle(int res) {
    ...
    @Override
    public MyFragmentTransaction setBreadCrumbTitle(CharSequence text) {
    ...
    @Override
    public MyFragmentTransaction setBreadCrumbShortTitle(int res) {
    ...
    @Override
    public MyFragmentTransaction setBreadCrumbShortTitle(CharSequence text) {
    ...
    void bumpBackStackNesting(int amt) {
        ...
        if (MyFragmentManagerImp.DEBUG) Log.v(TAG, "Bump nesting in " + this
                + " by " + amt);
        final int numOps = mOps.size();
        for (int opNum = 0; opNum < numOps; opNum++) {
            final Op op = mOps.get(opNum);
            if (op.fragment != null) {
                ...
                if (MyFragmentManagerImp.DEBUG) Log.v(TAG, "Bump nesting of "
    ...
    @Override
    public MyFragmentTransaction setAllowOptimization(boolean allowOptimization) {
    ...
    /**
     * Implementation of {@link MyFragmentManagerImp.OpGenerator}.
     ...
     */
    @Override
    public boolean generateOps(ArrayList<MyBackStackRecord> records, ArrayList<Boolean> isRecordPop) {
        if (MyFragmentManagerImp.DEBUG) {
            Log.v(TAG, "Run: " + this);
        }
    ...
    boolean interactsWith(ArrayList<MyBackStackRecord> records, int startIndex, int endIndex) {
        ...
        for (int opNum = 0; opNum < numOps; opNum++) {
            ...
            if (container != 0 && container != lastContainer) {
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    MyBackStackRecord record = records.get(i);
    ...
    void executeOps() {
        ...
        for (int opNum = 0; opNum < numOps; opNum++) {
            final Op op = mOps.get(opNum);
            final MyFragment f = op.fragment;
    ...
    void executePopOps(boolean moveToState) {
        for (int opNum = mOps.size() - 1; opNum >= 0; opNum--) {
            ...
            f.setNextTransition(MyFragmentManagerImp.reverseTransit(mTransition), mTransitionStyle);
            switch (op.cmd) {
                case OP_ADD:
     ...
     */
    void expandReplaceOps(ArrayList<MyFragment> added) {
     ...
     *              after the execution ({@link #executeOps()}.
     */
    void trackAddedFragmentsInPop(ArrayList<MyFragment> added) {
        for (int opNum = 0; opNum < mOps.size(); opNum++) {
            final Op op = mOps.get(opNum);
            switch (op.cmd) {
                case OP_ADD:
    ...
    void setOnStartPostponedListener(MyFragment.OnStartEnterTransitionListener listener) {
        for (int opNum = 0; opNum < mOps.size(); opNum++) {
            final Op op = mOps.get(opNum);
            if (isFragmentPostponed(op)) {
    ...
    private static boolean isFragmentPostponed(Op op) {
        final MyFragment fragment = op.fragment;
...
final class BackStackState implements Parcelable {
    ...
    public BackStackState(MyBackStackRecord bse) {
        ...
        int pos = 0;
        for (int opNum = 0; opNum < numOps; opNum++) {
            final MyBackStackRecord.Op op = bse.mOps.get(opNum);
            mOps[pos++] = op.cmd;
    ...
    public MyBackStackRecord instantiate(MyFragmentManagerImp fm) {
        MyBackStackRecord bse = new MyBackStackRecord(fm);
        int pos = 0;
        int num = 0;
        while (pos < mOps.length) {
            MyBackStackRecord.Op op = new MyBackStackRecord.Op();
            op.cmd = mOps[pos++];
            if (MyFragmentManagerImp.DEBUG) Log.v(MyFragmentManagerImp.TAG,
                    "Instantiate " + bse + " op #" + num + " base fragment #" + mOps[pos]);
            int findex = mOps[pos++];
            if (findex >= 0) {
                MyFragment f = fm.mActive.get(findex);
                op.fragment = f;
            } else {
                op.fragment = null;
            }
